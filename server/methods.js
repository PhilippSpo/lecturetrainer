...
Meteor.methods({
  giveAnswer: function (doc) {
    ...
    correctAnswers = removeDiacritics(correctAnswers).replace(/ /g, '').toLowerCase()
      .split(',');
    var userAnswers = removeDiacritics(doc.answer).replace(/ /g, '').toLowerCase()
      .split(',');
  ...
  correctAnswer: function (projectId, chapterId, questionId) {
    var doc = {
  ...
  wrongAnswer: function (projectId, chapterId, questionId) {
    var doc = {
      projectId: projectId,
      questionId: questionId,
      chapterId: chapterId
  ...
  calcNextQuestion: function (projectId, chapterId, questionId) {
    var question = findQuestionWithoutScore(questionId, chapterId, this.userId);
    if (question) {
      return question._id;
    }
    ...
    if (random > 80) {
      ...
      rating = questionWithPositiveScore(questionId, chapterId);
    }
    if (random <= 80 || !rating) {
      // go get one with score < 0
      rating = questionWithNegativeScore(questionId, chapterId);
      if (!rating) {
        rating = questionWithPositiveScore(questionId, chapterId);
      }
    }
...
function correctAnswer(userId, doc) {
  var currentRating = Ratings.findOne({
    user: userId,
...
function findQuestionWithoutScore(currentQuestion, chapter, userId) {
...
function questionWithPositiveScore(currentQuestion, chapterId) {
...
function questionWithNegativeScore(currentQuestion, chapterId) {
  var ratings = Ratings.find({
    chapter: chapterId,
    score: {
      $lt: 3
